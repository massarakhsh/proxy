...
func BuildFile(rule ruler.DataRuler, level int, path []string) Filer {
	...
	it.IfMarshal = func (drive likapi.DataDriver, status string) {
		it.ItMarshal(drive.(ruler.DataRuler), status)
	}
...
func (it *FileControl) ShowMenu(rule ruler.DataRuler) likdom.Domer {
	level,_ := rule.SeekControl(it.Index)
	tbl := likdom.BuildTableClassId("menu", "menu_" + it.Index)
	row := tbl.BuildTr()
	namely := "Корневая папка"
	if name := it.calcKeyName(rule, level); name != "" {
		namely = name
	}
	it.MenuItemCmd(rule, row, "", namely, "seek")
	if page := rule.GetItPage(); page != nil {
		if level+1 == len(page.Stack) {
...
func (it *FileControl) ItExecute(rule ruler.DataRuler, path []string) {
...
func (it *FileControl) ItMarshal(rule ruler.DataRuler, status string) {
	if it.IsNeedRemenu {
...
func (it *FileControl) ShowFoto(rule ruler.DataRuler) likdom.Domer {
	...
	path := it.calcPath(rule, it.GetLevel(rule)) + "/" + it.KeySel
...
func (it *FileControl) execGridInit(rule ruler.DataRuler) {
	...
	grid.SetItem(rule.BuildUrl("/front/"+it.Index+"/griddata"), "ajax")
...
func (it *FileControl) execGridData(rule ruler.DataRuler) {
	...
	if it.Mode != "last" {
		pots = it.readDir(rule, it.GetLevel(rule))
	...
	if pots != nil {
		...
		for n := 0; n < it.Length && it.Start+n < it.Total; n++ {
			...
			if pot.IsDir {
				...
				path := dirMain[1:] + it.calcPath(rule, it.GetLevel(rule)) + "/" + pot.Name
...
func (it *FileControl) execOpen(rule ruler.DataRuler) {
	...
	if key != "" {
		path := dirMain + it.calcPath(rule, it.GetLevel(rule)) + "/" + key
		if stat, err := os.Stat(path); err == nil {
			if stat.IsDir() {
				it.Mode = "dir_" + key
				BuildFile(rule, it.GetLevel(rule)+1, nil)
				rule.SetNeedRedraw()
			}
		}
	}
...
func (it *FileControl) execFolder(rule ruler.DataRuler) {
	...
	path := dirMain + it.calcPath(rule, it.GetLevel(rule)) + "/" + name
...
func (it *FileControl) execRename(rule ruler.DataRuler) {
	if key := rule.Shift(); key != "" {
		if name := lik.Transliterate(lik.StringFromXS(rule.Shift())); name != "" {
			pathold := dirMain + it.calcPath(rule, it.GetLevel(rule)) + "/" + key
			pathnew := dirMain + it.calcPath(rule, it.GetLevel(rule)) + "/" + name
	...
	if page := rule.GetItPage(); page != nil {
		for _, pot := range page.UpList {
			if pot.Data != nil {
				...
				//}
				path := dirMain + it.calcPath(rule, it.GetLevel(rule)) + "/" + file
...
func (it *FileControl) calcKeyName(rule ruler.DataRuler, level int) string {
	...
	if level > 0 {
		page := rule.GetItPage()
		mode := page.Stack[level-1].GetMode()
		if match := lik.RegExParse(mode, "^dir_(.+)$"); match != nil {
			name = match[1]
		}
	}
