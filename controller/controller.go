...
type Controller interface {
	likapi.Controller
	ShowMenu(rule ruler.DataRuler) likdom.Domer
	ShowInfo(rule ruler.DataRuler) likdom.Domer
	ItExecute(rule ruler.DataRuler, path []string)
}

func (it *DataControl) BuildPart(part string) string {
	return "/" + it.Index + "/" + part
}
...
func (it *DataControl) MenuPrepare(rule ruler.DataRuler, state bool) likdom.Domer {
	id := fmt.Sprintf("menu_%s", it.Index)
	tbl := likdom.BuildTableClass("menu", "id", id)
	if level,_ := rule.SeekControl(it.Index); level >= 0 {
		if !state && level+1 >= rule.GetLevel() {
			it.Mode = ""
		}
	}
	return tbl
...
func (it *DataControl) GetLevel(rule ruler.DataRuler) int {
	lev, _ := rule.SeekControl(it.Index)
	return lev
}
...
func (it *DataControl) ExecuteController(rule ruler.DataRuler, cmd string) {
	if level,_ := rule.SeekControl(it.Index); level >= 0 {
		if cmd == "exit" {
			rule.SetControl(level, nil)
		} else if cmd == "seek" {
			rule.SetControl(level+1, nil)
		}
	}
}
