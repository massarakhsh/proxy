...
func (it *Proxy) ItExecute(rule ruler.DataRuler, path []string) {
...
func (it *Proxy) ItMarshal(rule ruler.DataRuler, status string) {
	it.serverControl()
...
func (it *Proxy) execGridRefresh(rule ruler.DataRuler, path []string) {
	...
	it.ClientIndex = lik.StrToInt(it.PopCommand(&path))
	it.ClientLength = lik.StrToInt(it.PopCommand(&path))
