...
func BuildGate(rule ruler.DataRuler, level int, path []string) likapi.Controller {
	it := &Control{}
...
func (it *Control) ItExecute(rule ruler.DataRuler, path []string) {
	if cmd := it.PopCommand(&path); cmd == "" {
	} else if cmd == "summary" {
		it.Mode = "summary"
		rule.SetControl(it.GetLevel(rule)+1, nil)
	} else if cmd == "station" {
		it.Mode = "station"
		BuildStation(rule, it.GetLevel(rule)+1, path)
	} else if cmd == "proxy" {
		it.Mode = "proxy"
		BuildProxy(rule, it.GetLevel(rule)+1, path)
	} else {
